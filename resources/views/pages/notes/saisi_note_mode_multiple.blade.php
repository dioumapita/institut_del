                                ...
                                <table
                                    ...
                                    <thead>
                                        <tr>
                                            <th> Matricule </th>
                                            <th> Prénom et Nom </th>
                                            @if($niveau_choisi->status == 'PRIMAIRE')
                                                @foreach ($niveau_choisi->matieres as $matiere)
                                                    <th class="text-center">{{ $matiere->nom_matiere  }}</th>
                                                @endforeach
                                            @endif
                                            @if($niveau_choisi->status == 'SECONDAIRE')
                                                ...
                                                @if($niveau_choisi->status == 'PRIMAIRE')
                                                    @foreach ($niveau_choisi->matieres as $matiere)
                                                        <td class="text-center">
                                                            ...
                                                            <input type="number" name="composition[]" id="composition" min="0" max="{{ $bareme }}" step="0.01"
                                                                value="{{ $inscrits->eleve->notes->where('matiere_id',$matiere->id)->where('trimestre_id',$trimestre_choisit->id)->where('annee_id',$annee_choisie->id)->pluck('composition')->first() }}"
                                                                style="width: 9em">
                                                        </td>
                                                    @endforeach
                                                @endif
