...
@section('content')
    <div class="page-bar">
        <div class="page-title-breadcrumb">
            <div class=" pull-left">
                <div class="page-title">Etat De Paiement Des Etudiants De La {{ $niveau->nom_niveau.' '.$niveau->options }}</div>
            </div>
            <ol class="breadcrumb page-breadcrumb pull-right">
                ...
                <li class="active">Etat De Paiement Des Etudiants  De La {{ $niveau->nom_niveau.' '.$niveau->options }}</li>
            </ol>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="card card-topline-red">
                <div class="card-head">
                    <header>
                        @if($etat == 1)
                            Etat De Paiement Des Etudiants De La {{ $niveau->nom_niveau.' '.$niveau->options }}
                        @elseif($etat == 2)
                            Etudiant En Rétard De Paiement De La {{ $niveau->nom_niveau.' '.$niveau->options }}
                        @elseif($etat == 3)
                            Etudiant En Régle De Paiement De La {{ $niveau->nom_niveau.' '.$niveau->options }}
                        @elseif($etat == 4)
                            Etudiant En Rétard De Paiement De La Première Tranche De La {{ $niveau->nom_niveau.' '.$niveau->options }}
                        @elseif($etat == 5)
                            Etudiant En Rétard De Paiement De La Deuxième Tranche  De La {{ $niveau->nom_niveau.' '.$niveau->options }}
                        @else
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-6 col-6">
                            ...
                            <div class="btn-group">
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div  class="modal-header btn btn-danger text-center text-white">
                                                    <h4 class="modal-title white-text w-100 font-weight-bold py-2">Etat De Paiement Des Etudiants</h4>
                                                            ...
                                                            <form action="{{ route('etat_paiement_eleve_par_classe') }}" method="post">
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    <select class="form-control" name="status" id="status">
                                                                        ...
                                                                        <option value="4" @if($etat == 4) selected @endif>Retard de paiement de la 1ère tranche</option>
                                                                        <option value="5" @if($etat == 5) selected @endif>Retard de paiement de la 2ème tranche</option>
                                                                    </select>
                                                                </div>
                                ...
                                <!-- fin modal -->
                                &nbsp;
                                <a id="media_screen"  href="#" onclick="printDiv('imprime')" class="btn btn-primary"><i class="fa fa-print"></i> Imprimer</a>
                                &nbsp;
                                ...
                                @foreach ($all_inscriptions as $inscription)
                                    @if ($etat == 1)
                                        <tr class="odd gradeX">
                                            ...
                                            <td>{{ number_format(($inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->scolarite - $inscription->eleve->remisePaiementEleves->where('annee_id',$annee_courante->id)->sum('montant_reduit')),0,',',' ')." GNF" }} </td>
                                            <td>
                                                {{ number_format($inscription->eleve->paiementEleves->where('annee_id',$annee_courante->id)->where('status',1)->sum('somme_payer'),0,',',' ')." GNF" }}
                                            </td>
                                            <td>
                                                {{ number_format($inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->scolarite - ($inscription->eleve->paiementEleves->where('annee_id',$annee_courante->id)->where('status',1)->sum('somme_payer') + $inscription->eleve->remisePaiementEleves->where('annee_id',$annee_courante->id)->sum('montant_reduit')),0,',',' '). " GNF" }}
                                            </td>
                                        </tr>
                                    @elseif($etat == 2)
                                    ...
                                    @elseif($etat == 4)
                                        ...
                                        @if($inscription->status == 0)
                                            @if(($inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->tranche1) > ($inscription->eleve->paiementEleves->where('annee_id',$annee_courante->id)->where('status',1)->sum('somme_payer') + $inscription->eleve->remisePaiementEleves->where('annee_id',$annee_courante->id)->sum('montant_reduit')))
                                                <tr class="odd gradeX">
                                                    <td class="left">{{ $inscription->eleve->matricule }}</td>
                                                    <td>{{ $inscription->eleve->nom.' '.$inscription->eleve->prenom }}</td>
                                                    <td>{{ number_format(($inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->scolarite - $inscription->eleve->remisePaiementEleves->where('annee_id',$annee_courante->id)->sum('montant_reduit')),0,',',' ')." GNF" }} </td>
                                            ...
                                            @endif
                                        @else
                                            @if(($inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->tranche1_reinscription) > ($inscription->eleve->paiementEleves->where('annee_id',$annee_courante->id)->where('status',1)->sum('somme_payer') + $inscription->eleve->remisePaiementEleves->where('annee_id',$annee_courante->id)->sum('montant_reduit')))
                                                <tr class="odd gradeX">
                                                    <td class="left">{{ $inscription->eleve->matricule }}</td>
                                                    <td>{{ $inscription->eleve->nom.' '.$inscription->eleve->prenom }}</td>
                                                    <td>{{ number_format(($inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->scolarite - $inscription->eleve->remisePaiementEleves->where('annee_id',$annee_courante->id)->sum('montant_reduit')),0,',',' ')." GNF" }} </td>
                                        ...
                                        @endif
                                    @elseif($etat == 5)
                                        @if(($inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->tranche1 + $inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->tranche2) > ($inscription->eleve->paiementEleves->where('annee_id',$annee_courante->id)->where('status',1)->sum('somme_payer') + $inscription->eleve->remisePaiementEleves->where('annee_id',$annee_courante->id)->sum('montant_reduit')))
                                            <tr class="odd gradeX">
                                                <td class="left">{{ $inscription->eleve->matricule }}</td>
                                                <td>{{ $inscription->eleve->nom.' '.$inscription->eleve->prenom }}</td>
                                                <td>{{ number_format(($inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->scolarite - $inscription->eleve->remisePaiementEleves->where('annee_id',$annee_courante->id)->sum('montant_reduit')),0,',',' ')." GNF" }} </td>
                    ...
                    <div class="col-md-12">
                                    ...
                                    @foreach ($all_inscriptions as $inscription)
                                        @if ($etat == 1)
                                            <tr class="text-center" id="bordure_table">
                                                ...
                                                <td class="text-center" id="bordure_table">{{ number_format(($inscription->niveau->frais_scolaires->where('annee_id',$annee_courante->id)->first()->scolarite - $inscription->eleve->remisePaiementEleves->where('annee_id',$annee_courante->id)->sum('montant_reduit')),0,',',' ')." GNF" }} </td>
                        ...
                        <div class="">
                            <h4 class=" pull-right font-bold addr-font-h4">
                             Labé, le {{ date('d/m/Y') }}<br><br>La Comptabilité
                            </h4>
                        </div>
                    </div>
                </div>
