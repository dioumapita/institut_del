                            ...
                            <tbody>
                                ...
                                @foreach ($all_enseignant as $enseignant)
                                    <tr class="odd gradeX">
                                        ...
                                        <td>
                                            @if($enseignant->paiementEnseignants->where('mois_paiement',$num_mois)->sum('salaire_du_mois') > 0)
                                                {{ number_format(
                                                    (
                                                        $enseignant->paiementEnseignants->where('mois_paiement',$num_mois)->sum('salaire_du_mois')
                                                    )
                                                    ,0,'.',' ').' GNF'
                                                }}
                                            @else
                                                            ...
                                                            <div class="modal-body">
                                                                <div class="center">
                                                                    ...
                                                                    <form action="{{ route('paiement_enseignant.store') }}" method="post">
                                                                        ...
                                                                        <input type="hidden" name="salaire_du_mois" value="{{ $enseignant->salaire - ($enseignant->creditEnseignants->where('type_de_credit','Bon')->where('mois_remboursement',$num_mois)->sum('somme_credit') + $enseignant->creditEnseignants->where('type_de_credit','Prêt')->where('debut_remboursement','<=',$num_mois)->where($num_mois,'<=','fin_remboursement')->sum('montant_par_mois')) }}">
                                                                        @if($enseignant->paiementEnseignants->where('mois_paiement',$num_mois)->sum('salaire_du_mois') > 0)
                                                                            <div class="form-group">
                                                                                <label for="montant">Montant Payer *</label>
                                                                                <input type="number"
                                                                                class="form-control" name="montant" id="montant" aria-describedby="helpId" placeholder="" required min="0" max="{{ $enseignant->paiementEnseignants->where('mois_paiement',$num_mois)->sum('salaire_du_mois') - ($enseignant->creditEnseignants->where('type_de_credit','Bon')->where('mois_remboursement',$num_mois)->sum('somme_credit') + $enseignant->creditEnseignants->where('type_de_credit','Prêt')->where('debut_remboursement','<=',$num_mois)->where($num_mois,'<=','fin_remboursement')->sum('montant_par_mois')) - ($enseignant->paiementEnseignants->where('mois_paiement',$num_mois)->sum('somme_payer')) }}">
                                                                            </div>
                                                                        @else
                                                                            <div class="form-group">
                                                                                <label for="montant">Montant Payer *</label>
                                                                                <input type="number"
                                                                                class="form-control" name="montant" id="montant" aria-describedby="helpId" placeholder="" required min="0" max="{{ $enseignant->salaire - ($enseignant->creditEnseignants->where('type_de_credit','Bon')->where('mois_remboursement',$num_mois)->sum('somme_credit') + $enseignant->creditEnseignants->where('type_de_credit','Prêt')->where('debut_remboursement','<=',$num_mois)->where($num_mois,'<=','fin_remboursement')->sum('montant_par_mois')) - ($enseignant->paiementEnseignants->where('mois_paiement',$num_mois)->sum('somme_payer')) }}">
                                                                            </div>
                                                                        @endif
                                                                        <div class="form-group">
